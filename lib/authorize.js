...
function variations(parts) {
	if (parts.length === 1) {
		return parts[0];
	}
	var subvariations = variations(parts.slice(1));
	var result = { };
	parts[0].forEach(function (version) {
		subvariations.forEach(function (subVersion) {
			if (version !== null && subVersion !== null) {
				result[version + subVersion] = true;
			}
		});
	});
...
function authorize(req, res, next) {
	...
	if (client.signatureRequired) {
		...
		var ops = req.params.operations;
		var url = req.params.imageUrl;
		var urlSingleSlash = url.replace(/(https?):\/\/?/g, '$1:/');
		var urlDoubleSlash = url.replace(/(https?):\/\/?/g, '$1://');

		var firstSlashPos = req.url.indexOf('/', 1);
		var urlAfterClient = req.url.substr(firstSlashPos + 1);
		var urlAfterClientSingleSlash = urlAfterClient.replace(/(https?):\/\/?/g, '$1:/');
		var urlAfterClientDoubleSlash = urlAfterClient.replace(/(https?):\/\/?/g, '$1://');

		var versions = [
			urlAfterClient,
			urlAfterClientSingleSlash,
			urlAfterClientDoubleSlash
		].concat(variations([
			[
				ops,
				tryEncode(ops),
			...
			[
				...
				urlDoubleSlash,
				tryEncode(url),
				tryEncode(urlSingleSlash),
				tryEncode(urlDoubleSlash),
				tryDecode(url),
				tryDecode(urlSingleSlash),
				tryDecode(urlDoubleSlash)
			]
		]));

		var invalid = true;
		for (var i = 0; i < versions.length; i++) {
			if (client.verifySignature(req.params.signature, versions[i])) {
				invalid = false;
				break;
			}
		}
