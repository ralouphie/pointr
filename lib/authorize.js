...
var rateLimitUniqueBuilder = {
	ip: function rateLimitUniqueBuilderIp(req) {
		return (rateLimiterIpHeader && req.header(rateLimiterIpHeader)) || req.ip;
	}
};
...
function authorize(req, res, next) {
	...
	if (client.signatureRequired) {
		...
		if (!client.verifySignature(req.params.signature, req.params.operations + '/' + req.params.imageUrl)) {
